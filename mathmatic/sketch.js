...
const fontOptions = [
  { key: 'gaisyr', label: 'Gaisyr', path: 'libraries/ABCGaisyr-Bold.otf' },
  { key: 'oracle', label: 'Oracle', path: 'libraries/ABCOracle-Bold.otf' },
  { key: 'pareto', label: 'Pareto', path: 'libraries/ABCPareto-Rectangular.otf' }
];

function preload() {
  for (let option of fontOptions) {
    myFonts[option.key] = loadFont(option.path);
  }
  myFont = myFonts[currentFontKey];
}
...
function extractContours() {
  ...
  function appendLinePoints(points, target) {
    ...
    let dx = target.x - p0.x;
    let dy = target.y - p0.y;
      ...
      if (cmd.type === 'M') {
        ...
        if (current.length > 1) {
          ...
          if (first.x !== last.x || first.y !== last.y) {
            appendLinePoints(current, first);
          }
...
function createFourierControls() {
  ...
  let fontGroup = createDiv('');
  fontGroup.parent(panel);
  fontGroup.class('slider-group');
  createDiv('Font').parent(fontGroup).class('slider-label');

  let fontBtnsWrap = createDiv('');
  fontBtnsWrap.parent(fontGroup);
  fontBtnsWrap.class('font-btns');

  function setFont(key) {
    currentFontKey = key;
    myFont = myFonts[key];
    for (let btnKey in fontBtns) {
      if (btnKey === key) {
        fontBtns[btnKey].addClass('active');
      } else {
        fontBtns[btnKey].removeClass('active');
      }
    }
    extractContours();
  }
  ...
  for (let option of fontOptions) {
    fontBtns[option.key] = createButton(option.label);
    fontBtns[option.key].parent(fontBtnsWrap);
    fontBtns[option.key].class('font-btn');
    if (option.key === currentFontKey) fontBtns[option.key].addClass('active');
    fontBtns[option.key].mousePressed(() => setFont(option.key));
  }
