...
function exposeLetterApi() {
  window.WrinkleLetters = {
    ...
    setAmount(value) {
      ...
      if (amountLabel) amountLabel.html(`Wrinkle `);
      redraw();
    },
    getText() {
      return letters.join("");
...
function sanitizeLetters(value) {
  const cleaned = String(value || "")
    .toUpperCase()
    .split("")
    .filter((letter) => SUPPORTED_LETTERS.includes(letter));
...
function buildGlyphs() {
  ...
  clipLayer.textSize(GLYPH_SIZE);
  clipLayer.textAlign(LEFT, BASELINE);

  const cols = min(GRID_COLS, letters.length);
  const rows = ceil(letters.length / cols);
  ...
  for (let i = 0; i < letters.length; i++) {
    ...
    const bbox = oracleFont.textBounds(letter, 0, 0, GLYPH_SIZE);
    const col = i % cols;
    const row = floor(i / cols);
    const centerX = cellW * (col + 0.5);
    const centerY = topMargin + cellH * (row + 0.52);
    const x = centerX - bbox.w / 2 - bbox.x;
    const y = centerY - bbox.h / 2 - bbox.y;
    const glyph = {
      letter,
      x: x + bbox.x,
      y: y + bbox.y,
...
function drawWrinkleLayer(amount) {
  const layerCtx = wrinkleLayer.drawingContext;
  layerCtx.save();
